...
async function openAddPlayModal() {
  ...
  try {
    ...
    if (gamesSnapshot.empty) {
      ...
      // Map and validate games
      let gamesArray = gamesSnapshot.docs.map((doc) => {
        const data = doc.data();
        const game = {
          id: doc.id,
          name: data.name || `[Unnamed Game - ${doc.id}]`  // Provide fallback for missing names
        };
...
// Helper function to format player lists more compactly on mobile
function formatPlayerList(players) {
  // On mobile, use slightly more compact formatting but don't truncate
