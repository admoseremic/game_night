...
All stats are computed from the `plays` list, filterable by time period. Reference implementations are in `Game Night.dc.html` (`leaderboard()`, `recordFor()`, `computePick()`, `currentStreak()`, `hallData()`, `gameDetail()`, `playerDetail()`).
...
- Two dropdown **pills**: **Period** and **Sort by** — each opens a bottom sheet. (Replaced older stacked chip rows.)
- Ranked player rows: medal/rank chip, avatar, name, `{plays} plays · {winPct}% win`, and the active sort stat big on the right.
...
### 6. Player Profile
Per‑person page (reachable from roster and leaderboard rows): their record, favorite/most‑played games, best wins, current form/streak, per‑game breakdown. (See `playerDetail()` for exact fields surfaced.)

### 7. Hall of Fame ("All‑time")  ⭐ headline ask
A celebratory records experience (not just a date filter):
- **The G.O.A.T** — all‑time leader card (wins / w.wins / beaten).
- **Records & milestones** — Biggest win (most players beaten), Longest win streak, Most‑played game, Longest‑standing record.
- **Rivalries** — head‑to‑head: two players, each one's wins, a split bar, total meetings, who leads (auto "nemesis"). Pairs need ≥ 5 shared games.
- **Trophy case** — record score per game with holder.
...
## Files
- `Game Night.dc.html` — full prototype + all stat algorithms (read `class Component`: `leaderboard`, `recordFor`, `computePick`, `currentStreak`, `gameDetail`, `playerDetail`, `hallData`, `saveLog`, `deletePlay`).
